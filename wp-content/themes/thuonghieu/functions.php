...
if ( ! function_exists( 'thienkhue_setup' ) ) :
	...
	function thienkhue_setup() {
		...
		register_nav_menus( array(
			...
			'menu-video-customer' => esc_html__( 'menu video customer', 'thienkhue' ),
			'menu-archive' => esc_html__( 'menu archive consultant', 'thienkhue' )
		) );
...
function thienkhue_widgets_init() {
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar why choose', 'thienkhue' ),
		'id'            => 'slider-whychoose',
		'description'   => esc_html__( 'Add widgets here.', 'thienkhue' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Slide quiz', 'thienkhue' ),
		'id'            => 'slider-quiz',
		'description'   => esc_html__( 'Add widgets here.', 'thienkhue' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
...
require_once ( dirname( __FILE__ ) . '/wp_bootstrap_navwalker_video.php') ;
require_once ( dirname( __FILE__ ) . '/wp_bootstrap_navwalker_archive.php') ;
...
function load_scripts_custom() {
	...
	if(is_front_page()|| is_home()) {
		wp_enqueue_script('youtube-home', get_template_directory_uri() . '/js/youtube.js', array(), '0.1.2', true );
		wp_enqueue_script('custom-home', get_template_directory_uri() . '/assets/js/custom-home.js', array(), '0.0.4.0', true );
		wp_enqueue_script('video-home', get_template_directory_uri() . '/video/js/video.js', array(), '0.2.0.9', true );
    } 
    if(is_single() || is_page() && !is_page_template(array('page-template.php'))) {
    ...
    if(is_singular( 'video' )){
    	wp_enqueue_script('single-youtube', get_template_directory_uri() . '/js/single-video.js', array(), '0.1.7', true );
    }  
}
